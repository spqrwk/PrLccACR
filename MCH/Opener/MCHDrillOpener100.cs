...
public class MCHDrillOpener100 : IOpener
{
    public string OpenerName => "MCH_钻头起手100";
    ...
    public void InitializeCountdown(CountDownHandler countdownHandler)
    {
        ...
        countdownHandler.AddAction(MCHSettings.Instance.GrabItLimit, G(MCHSkill.钻头));
    }
    ...
    public List<PAction> InCombatSequence
    {
        get
        {
            if (!MCHOpenerChecker.CheckOpenerOutbreakSpells(MCHSkill.钻头)) return new List<PAction>();
            var s = new List<PAction>();
            if (ApiHelper.技能可用(MCHSkill.钻头) && ApiHelper.技能已解锁(MCHSkill.钻头))
            {
                if (!ApiHelper.玩家有状态(MCHBuff.整备预备)) s.Add(O(MCHSkill.整备, ActionTargetType.Self));
                s.Add(G(MCHSkill.钻头));
            }
            s.Add(O(MCHSkill.将死)); s.Add(O(MCHSkill.双将));
            s.Add(G(MCHSkill.空气锚)); s.Add(O(MCHSkill.枪管加热, ActionTargetType.Self));
